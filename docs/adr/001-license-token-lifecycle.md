...
- Sign-out sets `signed_out`, `telemetry_disabled` (this plugin's global
  upload pause) and a new `auth_generation` in the credential file; the
  consent journal reads the generation. The Claude plugin sets `signed_out`
  and purges its audit queue. The purge of unsent data on sign-out and 402
...
- Re-activation without a stored token still exists and is not bound to the
  ...
  plugin; the broker sign-in for this plugin follows the shared auth work.
