...
Next action: resolve hook acceptance with verified actual cwd or a separately reviewed exact-task-ID-bound probe. Do not broaden collection to the workspace root, ask for logs, or claim automatic capture succeeded. The real document task need not be repeated to prove transcript format. Work origin preservation and inner exec semantics are now concrete adapter requirements.
