...
The expiry gap is repaired in source commit `973d1f4dce4b00f112b0a5c239566890c28c1944`. Existing task grants retain queued data and keep capturing through token expiry, including a restarted Stop hook. Same-user renewal does not extend consent. New grants require an unexpired credential; malformed identity, owner/device/tenant changes, logout and terminal purge still revoke. Both readiness probes now pass. This adapter still has no sender or refresh writer.

The exact repaired package was installed with the supported Codex plugin CLI into a separate Codex home. All 101 installed source files match, and all 12 manifest hook commands passed under synthetic state with network/subprocess blocking. Credentials stayed unchanged during hooks; expiry retained chunks; cleanup removed payloads; the production queue stayed empty. This is command/package compatibility, not proof of native desktop hook discovery, trust or dispatch. The normal app installation was not changed. See `installed-package-check.json` and `check-installed-package.cjs`.

Pipeline follow-up `3abe812f98e625ff17e5719c816edfa31275ddcf` fixes empty-analysis and internal source identity; all 333 analyzer tests pass. A six-turn Work fixture reaches real report assembly and ingest schema validation, both alone and with unsupported companion input. Models are scripted and network blocked. Source identity and outer tools survive; agent/surface/originator now also survive internal parsing, classification and task analysis. A short fixture below ten blocks returns an insufficient-activity skip with zero model calls, no report write or ingest, and clean worker acknowledgement. Eligible companions still run. Prompts/rubric still target developers, so general Work scoring remains unvalidated. See the pipeline's `docs/work-analyzer-compatibility.md`.
...
2. Review the completed expiry retention fix, then validate the agreed cross-client contract, including authoritative 402 and mixed-client writes. Fresh-token validation remains a delivery requirement. Missing or corrupt identity must fail closed without writing credentials.
...
`npm run check` passed version/manifest validation, 453 Node tests and all 55 offline lifecycle scenarios. The focused Work/bridge run passed 39 overlapping tests. Both readiness probes pass. All 333 analyzer tests pass, with 18 existing deprecation warnings. Changed-file lint has zero new findings versus e17bf5f2; legacy lint is not fully clean. The Work compatibility test passes app-scoped lint/format checks. Installed package checks passed as described above. No production data or real model request was used. Work branches remain local, and existing PRs are unchanged.
