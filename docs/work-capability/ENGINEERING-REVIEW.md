...
The pipeline candidate now has three additional synthetic analyzer tests and all 321 analyzer tests pass. A six-turn Work fixture reaches real report assembly and ingest schema validation, both alone and with unsupported companion input. Models are scripted and network blocked. Structured source identity and outer tools survive, but analysis drops surface identity and its prompts/rubric still target developers. A short fixture is filtered below ten blocks; a separate full-job probe still produced an empty-task report after 13 scripted calls. Work scoring and empty-analysis outcomes remain unresolved. See the pipeline's `docs/work-analyzer-compatibility.md`.
