...
function update(dir, phase, code, progress, write) {
  ...
  const file = path.join(dir, `${phase}-status.json`), previous = read(file) || {}, at = new Date().toISOString();
...
function inspect(dir) {
  const capture = read(path.join(dir, "capture-status.json"));
  const delivery = read(path.join(dir, "delivery-status.json"));
