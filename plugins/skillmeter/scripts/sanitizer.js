...
// rules and path vocabulary unchanged; document the pin in docs/claude-parity.md.
...
function sanitizeRecord(record, salt) {
  ...
  let input = record, argumentsWereJson = false;
  ...
  if (result.value && typeof result.value === "object" && !Array.isArray(result.value)) result.value._sanitization = result.meta;
  if (argumentsWereJson) result.value.payload.arguments = JSON.stringify(result.value.payload.arguments);
  return result;
}
function sanitizeLine(record, salt) { return sanitizeRecord(record, salt).value; }
...
function redactDeep(value, redactions = [], salt) {
  ...
  if (result.value && typeof result.value === "object" && !Array.isArray(result.value)) delete result.value._sanitization;
  return result.value;
}
module.exports = {
  POLICY_VERSION: shared.POLICY_VERSION, SECRET_PLACEHOLDER, EMAIL_PLACEHOLDER:"[EMAIL]",
  redactString: shared.redactString, containsTier1: shared.containsSecret,
  hashHmac: shared.hashHmac, sanitizeEventData:sanitizeRecord, sanitizeLine,
  sanitizeTranscript, redactDeep,
};
